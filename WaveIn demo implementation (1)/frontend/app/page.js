...
export default function Home() {
  ...
  // Shared event catalogue so events created in the organizer also appear to fans.
  const [events, setEvents] = useState(EVENTS);

  // Called when an organizer creates an event — adds it to the shared catalogue.
  ...
  return (
    <main style={{ position: "relative", minHeight: "100vh" }}>
      ...
      {screen === "organizer" && <OrganizerView events={events} onCreateEvent={handleCreateEvent} />}
      {screen === "fan" && <FanView events={events} />}
